...
interface IPasswordRequirementsLazy {
    upper?: boolean;
    lower?: boolean;
    number?: boolean;
    special?: boolean;
 ...
 */
function hasBreaks(str: string): boolean {
...
*/
export function validateDomain(domain: string, useWildCards: boolean = false): boolean {
...
*/
export function validateIPAddress(ip: string, useWildCards: boolean = false): boolean {
  if (typeof ip !== "string") {
      return false;
  }
  ...
  re = /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$|^\s*((([0-9A-Fa-f]{1,4}:){7}([0-9A-Fa-f]{1,4}|:))|(([0-9A-Fa-f]{1,4}:){6}(:[0-9A-Fa-f]{1,4}|((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9A-Fa-f]{1,4}:){5}(((:[0-9A-Fa-f]{1,4}){1,2})|:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9A-Fa-f]{1,4}:){4}(((:[0-9A-Fa-f]{1,4}){1,3})|((:[0-9A-Fa-f]{1,4})?:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9A-Fa-f]{1,4}:){3}(((:[0-9A-Fa-f]{1,4}){1,4})|((:[0-9A-Fa-f]{1,4}){0,2}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9A-Fa-f]{1,4}:){2}(((:[0-9A-Fa-f]{1,4}){1,5})|((:[0-9A-Fa-f]{1,4}){0,3}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9A-Fa-f]{1,4}:){1}(((:[0-9A-Fa-f]{1,4}){1,6})|((:[0-9A-Fa-f]{1,4}){0,4}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(:(((:[0-9A-Fa-f]{1,4}){1,7})|((:[0-9A-Fa-f]{1,4}){0,5}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:)))(%.+)?\s*$|^(((2(5[0-5]|[0-4][0-9])|[01]?[0-9][0-9]?)\.){3}(2(5[0-5]|[0-4][0-9])|[01]?[0-9][0-9]?)(\/(3[012]|[12]?[0-9])))$/;
  return re.test(ip);
}


/**
 * Validates if a given IPv4 address is valid.
 * 
...
export function validateIPv4(ip: string): boolean {
    return validateIPAddress(ip, false) && ip.indexOf(".") !== -1 && ip.indexOf(":") === -1;
}
...
*/
export function validateWindowsPath(str: string, useWildCards: boolean = false): boolean {
  if (typeof str !== "string") {
      return false;
  }
...
*/
export function setPasswordRequirements(obj: Partial<IPasswordRequirementsLazy>): boolean {
  if (typeof obj.upper !== "undefined") {
      passwordRequirements.requireUpper = !!obj.upper;
  }
  if (typeof obj.lower !== "undefined") {
      passwordRequirements.requireLower = !!obj.lower;
  }
  ...
  if (passwordRequirements.requireMaxLength > 0 && passwordRequirements.requireMinLength > 0 && 
      passwordRequirements.requireMaxLength <= passwordRequirements.requireMinLength) {
...
*/
export function validatePassword(str: string, bRequireSpecial?: boolean, nMinLength?: number, nMaxLength?: number): Partial<IPasswordResult> {
  let ret: Partial<IPasswordResult> = {};
  if (typeof str !== "string") {
...
function copySchema(schema: ISchema | any): ISchema | any {
  if (!schema) {
      return schema;
  }
...
function validateSchema(schema: ISchema, value: any, depth: number = 0): ISchemaValidationResult {
  const result: ISchemaValidationResult = {
      valid: true,
      errors: []
  };
...
function getSchemaValidatorFunction(name: string): (input: any) => ISchemaValidationResult {
  return (input: any): ISchemaValidationResult => {
